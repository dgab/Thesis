...
namespace ConsoleNeuralNets
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bpn.Initialize(8, 15, 1);

            TrainingSet ts = new TrainingSet(bpn.Layers.InputLayer, bpn.Layers.OutputLayer);
            ts.Add(new TrainingSample(new List<double>() { 1, 0, 1, 0, 1, 0, 1, 0 }, new List<double> { 0.1 }));
            ts.Add(new TrainingSample(new List<double>() { 1, 1, 1, 1, 1, 1, 1, 1 }, new List<double> { 0.2 }));
            ts.Add(new TrainingSample(new List<double>() { 0, 0, 0, 0, 0, 0, 0, 0 }, new List<double> { 0.3 }));
            double error = 1;
            do
            {
                error = bpn.Train(ts);
